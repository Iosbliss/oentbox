document.addEventListener('DOMContentLoaded', () => {
  ...
  const cardMarkup = (video) => {
    ...
    return `<a class="youtube-card" href="/youtube/video.html?id=${encodeURIComponent(video.id)}">
    ...
    <span class="youtube-card__body">
      ...
      <span class="youtube-card__text">
        ...
        <span class="youtube-card__stats">${video.views ? `${escapeHtml(video.views)} views` : 'Views unavailable'}${video.likes ? ` · ${escapeHtml(video.likes)} likes` : ''}</span>
      </span>
    </span>
  </a>`;
  };
