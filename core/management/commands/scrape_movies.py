...
class Command(BaseCommand):
    ...
    def extract_category(self, title, description):
        ...
        # Specific categories must be checked before the generic TV-series markers.
        if any(word in text for word in ['anime', 'cartoon', 'animation']):
            return 'anime'
        elif any(word in text for word in ['korean', 'k-drama', 'kdrama']):
            return 'korean-drama'
        elif any(word in text for word in ['chinese', 'c-drama', 'cdrama']):
            return 'chinese-drama'
        elif any(word in text for word in ['japanese', 'j-drama', 'jdrama']):
            return 'japanese-drama'
        elif any(word in text for word in ['filipino', 'philippines']):
            return 'filipino-drama'
        elif any(word in text for word in ['turkish', 'turkey']):
            return 'turkish-drama'
        elif any(word in text for word in ['thai', 'thailand']):
            return 'thai-drama'
        elif any(word in text for word in ['nollywood', 'nigerian', 'ghana']):
            return 'nollywood'
        elif any(word in text for word in ['season', 'episode', 'series']):
            return 'tv-series'
        else:
            return 'hollywood'
